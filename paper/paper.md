...
`BayesFactor` package for Bayesian statistics [@Morey2020]. So one can naturally
ask why there needs to be another package that wraps around these packages.
...
others expect dataframes. Depending on whether it's a repeated measures design
...
their presence. Furthermore, if someone wishes to further utilize the objects
returned by these packages in their workflow, this is not straightforward either
because even functions from the same package can return a list, a matrix, an
array, a dataframe, etc., depending on the function. So on and so forth.
