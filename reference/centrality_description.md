...
`"statsExpressions"`. Unlike the hypothesis-testing functions, this one
returns one row **per level** of the grouping variable `x`, describing
the distribution of `y` within that level. The columns are:

- the grouping variable (named after `x`, e.g. `Species`)

- the requested centrality estimate (named after `y`, e.g.
  `Sepal.Length`): the mean (`type = "parametric"`), median
  (`"nonparametric"`), trimmed mean (`"robust"`), or MAP estimate
  (`"bayes"`)
...
- `conf.low`, `conf.high`: bounds of the confidence/credible interval
  for the centrality estimate
...
- `expression`: a list-column of pre-formatted
  ...
  containing the centrality estimate, ready to be used in `{ggplot2}`

- `n.expression`: a label combining the group name and its sample size
...
For more examples, see the [data frame output
