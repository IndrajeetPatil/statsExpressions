...
------------ | ----------- | -------------------- | -------- | ---  
`oneway_anova` (2 groups) | Parametric | Student's and Welch's *t*-test | Cohen's *d*, Hedge's *g* | <font color="green">$\checkmark$</font>
`oneway_anova` (> 2 groups) | Parametric | Fisher's and Welch's one-way ANOVA | $\eta^2_p, \omega^2_p$ | <font color="green">$\checkmark$</font>
`oneway_anova` (2 groups) | Non-parametric | Mann-Whitney *U*-test | *r* | <font color="green">$\checkmark$</font>
`oneway_anova` (> 2 groups) | Non-parametric | Kruskal-Wallis Rank Sum Test | $\epsilon^2$ | <font color="green">$\checkmark$</font>
`oneway_anova` (2 groups) | Robust | Yuen's test for trimmed means | $\xi$ | <font color="green">$\checkmark$</font>
`oneway_anova` (> 2 groups) | Robust | Heteroscedastic one-way ANOVA for trimmed means | $\xi$ | <font color="green">$\checkmark$</font>
`oneway_anova` (2 groups) | Parametric | Student's *t*-test | Cohen's *d*, Hedge's *g* | <font color="green">$\checkmark$</font>
`oneway_anova` (> 2 groups) | Parametric | Fisher's one-way repeated measures ANOVA | $\eta^2_p, \omega^2$ | <font color="green">$\checkmark$</font>
`oneway_anova` (2 groups) | Non-parametric | Wilcoxon signed-rank test | *r* | <font color="green">$\checkmark$</font>
`oneway_anova` (> 2 groups) | Non-parametric | Friedman rank sum test | $W_{Kendall}$ | <font color="green">$\checkmark$</font>
`oneway_anova` (2 groups) | Robust | Yuen's test on trimmed means for dependent samples | $\xi$ | <font color="green">$\checkmark$</font>
`oneway_anova` (> 2 groups) | Robust | Heteroscedastic one-way repeated measures ANOVA for trimmed means | <font color="red">$\times$</font> | <font color="red">$\times$</font>
`contingency_table` (unpaired) | Parametric | $\text{Pearson's}~ \chi^2 ~\text{test}$ | Cramér's *V* | <font color="green">$\checkmark$</font>
`contingency_table` (paired) | Parametric | McNemar's test | Cohen's *g* | <font color="green">$\checkmark$</font>
`contingency_table` | Parametric | One-sample proportion test | Cramér's *V* | <font color="green">$\checkmark$</font>
`corr_test` | Parametric | Pearson's *r* | *r* | <font color="green">$\checkmark$</font>
`corr_test` | Non-parametric | $\text{Spearman's}~ \rho$ | $\rho$ | <font color="green">$\checkmark$</font>
`corr_test` | Robust | Percentage bend correlation | *r* | <font color="green">$\checkmark$</font>
`one_sample_test` | Parametric | One-sample *t*-test | Cohen's *d*, Hedge's *g* | <font color="green">$\checkmark$</font>
`one_sample_test` | Non-parametric | One-sample Wilcoxon signed rank test | *r* | <font color="green">$\checkmark$</font>
`one_sample_test` | Robust | One-sample percentile bootstrap | robust estimator | <font color="green">$\checkmark$</font>
`meta_analysis` | Parametric | Meta-analysis via random-effects models | $\beta$ | <font color="green">$\checkmark$</font>
`meta_analysis` | Robust | Meta-analysis via robust random-effects models | $\beta$ | <font color="green">$\checkmark$</font>
...
**Function**: `effectsize::rank_biserial`
